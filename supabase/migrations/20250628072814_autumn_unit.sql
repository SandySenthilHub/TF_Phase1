-- TF_genie Database Complete Setup Script (Fixed Version)
...
-- Step 3: Drop existing tables if they exist (to fix constraint issues)
IF EXISTS (SELECT * FROM sysobjects WHERE name='TF_master_fields' AND xtype='U')
    DROP TABLE TF_master_fields;
IF EXISTS (SELECT * FROM sysobjects WHERE name='TF_key_value_pair' AND xtype='U')
    DROP TABLE TF_key_value_pair;
IF EXISTS (SELECT * FROM sysobjects WHERE name='TF_master_documentset' AND xtype='U')
    DROP TABLE TF_master_documentset;
IF EXISTS (SELECT * FROM sysobjects WHERE name='TF_master_record' AND xtype='U')
    DROP TABLE TF_master_record;
IF EXISTS (SELECT * FROM sysobjects WHERE name='ingestion_documents_fields_new' AND xtype='U')
    DROP TABLE ingestion_documents_fields_new;
IF EXISTS (SELECT * FROM sysobjects WHERE name='ingestion_documents_new' AND xtype='U')
    DROP TABLE ingestion_documents_new;
IF EXISTS (SELECT * FROM sysobjects WHERE name='ingestion_keyValuePair' AND xtype='U')
    DROP TABLE ingestion_keyValuePair;
IF EXISTS (SELECT * FROM sysobjects WHERE name='ingestion_fields' AND xtype='U')
    DROP TABLE ingestion_fields;
IF EXISTS (SELECT * FROM sysobjects WHERE name='ingestion_document_cleaned' AND xtype='U')
    DROP TABLE ingestion_document_cleaned;
IF EXISTS (SELECT * FROM sysobjects WHERE name='ingestion_document_raw' AND xtype='U')
    DROP TABLE ingestion_document_raw;
IF EXISTS (SELECT * FROM sysobjects WHERE name='ingestion_session' AND xtype='U')
    DROP TABLE ingestion_session;
IF EXISTS (SELECT * FROM sysobjects WHERE name='users' AND xtype='U')
    DROP TABLE users;
GO

-- Step 4: Create all tables with proper constraints
...
-- Cleaned documents table (fixed constraints)
CREATE TABLE ingestion_document_cleaned (
    ...
    FOREIGN KEY (sessionId) REFERENCES ingestion_session(id) ON DELETE NO ACTION
);
GO

-- Fields table (fixed constraints)
...
-- Key-value pairs table (fixed constraints)
CREATE TABLE ingestion_keyValuePair (
    ...
    createdAt DATETIME2 DEFAULT GETDATE(),
    FOREIGN KEY (sessionId) REFERENCES ingestion_session(id) ON DELETE CASCADE,
    FOREIGN KEY (documentId) REFERENCES ingestion_document_raw(id) ON DELETE NO ACTION
);
GO

-- New documents table (fixed constraints)
CREATE TABLE ingestion_documents_new (
    ...
    FOREIGN KEY (sessionId) REFERENCES ingestion_session(id) ON DELETE CASCADE,
    FOREIGN KEY (documentId) REFERENCES ingestion_document_raw(id) ON DELETE NO ACTION,
    FOREIGN KEY (requestedBy) REFERENCES users(id) ON DELETE NO ACTION,
...
-- Master key-value pairs table (fixed constraints)
CREATE TABLE TF_key_value_pair (
    ...
    createdAt DATETIME2 DEFAULT GETDATE(),
    FOREIGN KEY (masterRecordId) REFERENCES TF_master_record(id) ON DELETE CASCADE,
    FOREIGN KEY (documentSetId) REFERENCES TF_master_documentset(id) ON DELETE NO ACTION
);
GO
...
-- Step 6: Insert default users (password is 'password' for both)
