-- TF_genie Database Schema Creation Script
-- Run this script in your Azure SQL Server Management Studio
...
-- Create ingestion_session table
...
-- Create ingestion_document_raw table
...
-- Create ingestion_document_cleaned table
...
CREATE TABLE ingestion_document_cleaned (
    ...
    extractedFields NTEXT, -- JSON string of extracted fields
...
-- Create ingestion_keyValuePair table
...
-- Create ingestion_documents_new table (for unmatched documents)
...
-- Create ingestion_documents_fields_new table
...
-- Create TF_master_record table (final storage)
...
-- Create TF_master_documentset table
...
-- Create TF_key_value_pair table (final storage)
...
IF NOT EXISTS (SELECT * FROM users WHERE email = 'admin@tradefi.com')
...
IF NOT EXISTS (SELECT * FROM users WHERE email = 'user@tradefi.com')
INSERT INTO users (email, name, password, role) 
VALUES ('user@tradefi.com', 'Regular User', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'user');

PRINT 'TF_genie database schema created successfully!';